...
namespace TamborGame.UI
{
    public class DicesResultsController : APanelController
    {
        ...
        IEnumerator AnimateDicesResults()
        {
            ...
            {
                if (DiceProvider._Dice1Result != i)
                    imageDice1.sprite = diceFaceSpriteDictionary[i];


                if (DiceProvider._Dice2Result != i)
                    imageDice2.sprite = diceFaceSpriteDictionary[i];

                yield return new WaitForSeconds(animationSettings._Duration);
            }

            imageDice1.sprite = diceFaceSpriteDictionary[DiceProvider._Dice1Result];
            imageDice2.sprite = diceFaceSpriteDictionary[DiceProvider._Dice2Result];
        }
    }
}